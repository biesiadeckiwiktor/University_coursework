...
class Predator(Node):
    def __init__(self):
        ...
        # Get turtle name from parameter
        self.declare_parameter('turtle_name', 'predator1')
        self.turtle_name = self.get_parameter('turtle_name').value

        self.predator_publisher = self.create_publisher(Twist, f'/{self.turtle_name}/cmd_vel', 10)
        self.location_pub = self.create_publisher(Pose, f'/{self.turtle_name}_location', 10)
